...
| 062 | Notebook Info Page | **Done** — NotebookOverview component in main content area. Stats: total notes/folders (recursive), meeting minutes count, recent notes (top 5), last activity, creation date, access level. Triggered via "Info" in notebook context menu. | — |
...
| 063 | Corporate Vetting Flow | **Done** — EngagementVettingItem GQL entity (schema v1.0.8), VettingService with CRUD + lazy seeding of 6 default items + status transitions, Vetting tab (8th engagement tab) with expansion panel checklist, add item dialog. 18 unit tests. **Next:** Vetting gate UX — when all required items verified/waived, engagement status → "Verified" and project work can begin. Conditional items include "Pilot completed" (see Plan 077). Source: Brian 2026-03-27. | [`063-corporate-vetting.md`](../local/063-corporate-vetting.md) |
...
| **071** | **Transparency Entangled Task Pairs** | ⏸ **DEFERRED (Project App)** — Option B selected (design done). Implementation depends on platform Task/Board + entangled link types. **Update 2026-03-27:** Brian wants each task on each side to have a **"Transparency Controls" section** that publishes to the entangling bridge. UI spec can be drafted now. Source: Brian 2026-03-24, 2026-03-27. | [`071-entangled-task-pairs.md`](../local/071-entangled-task-pairs.md) |
...
| **075** | **RFP → Project Refactor** | **Done** — RFP = SmeMartProject in draft/published status. Bid acceptance gates on Engagement existence. Models, GQL types, field mappings, service layer, component migration, demo data, all 733 tests passing. | [`075-rfp-to-project-refactor.md`](../local/075-rfp-to-project-refactor.md) |
...
| **077** | **Pilot Projects** | **Stub** — Test/POC projects during engagement vetting. Provider demonstrates capability via a scoped pilot before the "real project" begins. Adds `projectType` field to SmeMartProject: `rfp \| pilot \| project`. Pilot completion becomes a conditional vetting item (Plan 063). Pilot → Real Project transition is a lifecycle event. Pilots have reduced scope expectations (budget, timeline). Source: Brian "PILOTS" 2026-03-27. ~6–8 hrs. | TBD |
| **078** | **Transparency Controls UI Spec** | **Stub** — Design spec for the "Transparency Controls" section on each party's tasks (Plan 071). Each task has controls that publish/subscribe to the transparency entangling bridge. Spec covers: what data is shared (status, progress, artifacts), granularity controls (per-field visibility), bridge visualization. Can be designed now, implementation deferred until platform Task/Board lands. Source: Brian 2026-03-27. ~4–6 hrs (spec only). | TBD |
