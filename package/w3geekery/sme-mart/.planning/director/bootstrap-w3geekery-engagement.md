...
- **W3Geekery walkthrough records originally had NO Object.tag applied** (we passed batch-level `tagIds` to Pipeline.receive, which doesn't tag Objects; we never populated `data[i].tag`). Original lean: accept for the proof-of-concept. Task↔Engagement bridging via shared `zerobiasTagId` + Step D's `tagResource` call still worked.
- **REMEDIATED 2026-04-27.** Re-ingested both records via Pipeline.receive with the validated `tag: [{value: "a81cd320-..."}]` payload. Engagement (`746010b7-...`) and default SmeMartProject (`ea4db55f-...`) now have Object.tag populated. Tag-filter discovery (`ClassName(tag: {value: ".eq.<id>"})`) works uniformly. See DECISIONS.md "W3Geekery Object.tag Remediation 2026-04-27".
