...
| **089** | zbb Hub Module Local Stack (Verdaccio + hub-server + postgres) | **Deferred from Phase 19 (2026-04-16).** Extends the Phase 19 cloudfront-sim + minio stacks with local Hub module hosting: (1) `registry` built-in (Verdaccio) for publishing unmerged SME Mart Hub module locally, (2) `hub-server` container loading the Hub module from local registry, (3) `postgres` built-in (Neon stand-in for hub-server). **Blocked on:** Kevin confirming hub-server runtime (Java? Node?), Docker image availability (ghcr.io or Dockerfile in `zerobias-org/hub`), and module loading mechanism (env vars, CLI flags, npm install from registry URL). **Why deferred:** SME Mart works fine without local Hub module — uses Neon direct + pipeline + GQL. Hub-server is the biggest unknown in Phase 19 and adds complexity without immediate payoff. Revisit when Hub module PR review queue becomes a bottleneck. **Depends on:** Phase 19 completion (cloudfront-sim + minio infrastructure). | ~4-6 hrs | Phase 19 scope reduction 2026-04-16 |
