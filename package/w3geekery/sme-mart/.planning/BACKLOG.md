...
| **091** | Provider card cosmetic polish (Phase 26 follow-up) | Two small nits surfaced during Phase 26 detail-page verify on 2026-04-28: (1) "Jobs completed" row missing the count prefix in the badge — should read like "12 jobs completed" but currently displays only the label. (2) Avatar placeholder text overflows the avatar circle when the display name is long (e.g., "ZeroBias" placeholder fits, but longer org names wrap or clip). Both contained to `provider-card.component.{ts,html,scss}`. Not Phase 26 closure-blocking — Director acknowledged. | 1–2 hrs | Phase 26 closeout 2026-04-28 |
