...
| **088** | Split-screen Form Builder + Info Field Type | **From Phase 16 UAT feedback 2026-04-14.** Replace the current "expand to edit one field at a time" form builder UX with a two-pane layout: left = live rendered preview of the field being edited (or whole form when none selected), right = field properties editor. Preview tab becomes a cleaner version of the left pane — full form rendered with proper spacing, read-only. **Also adds Option B** from the same UAT conversation: new `'info'` field type for heading + description text blocks that aren't inputs (not stored in submissionData, no validation). Drops anywhere between fields as explainer content. **Scope:** (1) Redesign `form-builder.component` as two-pane grid (mat-sidenav or CSS grid). (2) Selection model — clicking a field row highlights it + loads editor in right pane. (3) Live render — left pane uses `form-field-renderer` in new `edit-preview` mode (interactive, not disabled, but no persistence). (4) Add `'info'` to `FormFieldType` enum; `form-field-editor` shows only label + description (markdown-capable) for info fields; `dynamic-form-renderer` renders them as styled callout blocks; excluded from form-group controls + submissionData. (5) Preview tab uses existing `dynamic-form-renderer` with `mode='preview'` but with builder chrome stripped (just spaced form). (6) **"Add Field" split-button** — primary button label "Add Field" inserts a field of the most-recently-used type (or default); caret opens menu of all field types (text, textarea, dropdown, number, file, checkbox, info) and selecting one inserts a field of that type pre-configured. Saves two clicks per field when the user knows what type they want. **Dependencies:** Phase 16 components exist. **Prompt for `/gsd:plan-phase`:** *"Plan a split-screen Form Builder redesign for SME Mart Phase 16 follow-up, plus an 'info' (heading + description, no input) field type. Currently the form-builder shows a stacked list of expansion panels — one expanded at a time — for field editing. Redesign as two panes: left = live rendered preview of the currently-selected field (or whole form when none selected), right = that field's properties editor. Preview tab becomes the same left-pane render but full-form and clean. Also add FormFieldType 'info' for non-input explainer blocks with heading + optional markdown description, excluded from submissionData and validation. Cover: (a) form-builder.component.{ts,html,scss} two-pane layout (mat-sidenav or CSS grid), field selection model, responsive fallback for narrow viewports; (b) form-field-renderer — new 'edit-preview' mode that is interactive but non-persisting, plus 'info' field rendering as styled callout (not an input); (c) form-field-editor — detect 'info' type and show only label + description fields (no validation panel, no required toggle); (d) dynamic-form-renderer — skip info fields in form group creation, skip in submissionData collection, render as section callouts; (e) preview tab simplification — reuse dynamic-form-renderer with mode='preview' but ditch the builder chrome. Also replace the current single 'Add Field' button with a split-button: primary click inserts a field using the most-recently-used type (default to text on first use), caret opens a menu of all field types (text, textarea, dropdown, number, file, checkbox, info) to insert a pre-typed field. Reference existing Phase 16 components at src/app/shared/components/form-builder/. Out of scope: drag-and-drop reorder (already exists), template library (Plan 087), collaborative editing."* | 20–26 hrs | Phase 16 UAT feedback 2026-04-14 |
| **086** | zbb local stack — full prod simulation for SME Mart | Compose a `zbb` slot that mirrors the production SME Mart deployment locally so we can validate end-to-end before publishing is unlocked. **Stack composition:** (1) `postgres` built-in (Neon stand-in), (2) `minio` built-in (S3 stand-in for the published SPA bucket), (3) `registry` built-in (Verdaccio — host the unmerged SME Mart Hub module locally without waiting on upstream PR merge), (4) local `hub-server` loading the SME Mart Hub module from the local registry, (5) **NEW custom stack** `cloudfront-sim` — nginx in front of MinIO with SPA fallback (`try_files $uri /index.html`) to mirror the CloudFront → S3 published SPA serving path. Also covers the SME Mart **login** (`login/` repo) since it deploys to the same CloudFront/S3 path pattern. **Why now:** Sets us up to iterate on Hub module + published-shape SPA bugs (basePath, asset paths, route fallback) without depending on upstream PR review queues. When publishing IS unlocked, we already have the local stack proven. **Confirmed feasible by Kevin McCarthy 2026-04-13** ("you can make any stack you like that imports/exports env vars and exposes service ports"). **Reference:** `.claude/projects/.../memory/reference_zbb_local_stack.md`; design docs at `~/Projects/zb/zerobias-org/util/packages/zbb/design/{stacks-guide.md, stacks-spec.md, registry-spec.md}`. | ~7-10 hrs (Hub module hosting ~4-6 hrs + S3/CloudFront sim layer ~3-4 hrs) | Slack discussion w/ Kevin 2026-04-13 |
...
| **CE4** | Twinned Boundary Requirements | Every boundary requirement = Demand Twin + Supply Twin pair with independent RAG status and party-scoped visibility. Expands Plan 080. 14–18 hrs. |
...
### Resolved 2026-04-14 (Marketplace meeting w/ Brian — full answers in [research doc](.planning/research/external/2026-04-13-ceo-miro-cross-engagement-audit-model.md) + [meeting notes](.claude/notes/meetings/2026-04-14-marketplace.md))
