...
/**
 * Auto-fold every JSON container nested deeper than `maxDepth`.
 *
 ...
 *
 * Implemented as a ViewPlugin rather than a one-off call because `zb-code-editor` owns its
 * EditorView and does not expose it — an extension is the only way in from outside. It also has to
 * re-run on document change, since the same editor instance is reused when a new response lands.
 */
