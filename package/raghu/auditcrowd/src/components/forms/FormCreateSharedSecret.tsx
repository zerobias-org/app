...
import { SharedSessionKey, CreateSharedSessionKeyBody } from '@zerobias-com/dana-sdk';
...
export default function CreateSharedSessionKeyForm() {
  ...
  const createSharedSessionKey = async (minutes?: number): Promise<SharedSessionKey|undefined> => {
    const createSharedSessionKeyBody = new CreateSharedSessionKeyBody(
      undefined,
      minutes ? new Duration(minutesToDuration(minutes)) : new Duration('PT1440M'), // 24 hr default
    );
