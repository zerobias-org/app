...
import { ApiKey, CreateApiKeyBody } from '@zerobias-com/dana-sdk';
...
export default function CreateApiKeyForm() {
  ...
  const getApiKey = async () => {
    ...
    if (apiKeyName) {
      const body = new CreateApiKeyBody(apiKeyName, new DateTime(getFutureDate(10)));
      ...
      try{
        await instance
          ...
          .danaClient
          .getMeApi()
          .createApiKey(body).then((key: ApiKey & any) => {
            if (key) {
              /* 
                ApiKey & object (any):
                {
              ...
              */
              ...
              navigator.clipboard.writeText(key.data).then(() => {
                const currentOrgId:string = org?.id?.toString() ?? '';
                if (currentOrgId) {
